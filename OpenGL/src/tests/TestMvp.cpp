...
namespace test {
	TestMvp::TestMvp(GLFWwindow* window, ImGuiIO& io) : Test{ window, io }
	{
		...
		vb = VertexBuffer(quadrilateral.vertexes, sizeof(quadrilateral.vertexes));
		...
		ib = { quadrilateral.indices, sizeof(quadrilateral.indices) / sizeof(decltype(*quadrilateral.indices)) };
	...
	void TestMvp::onRender()
	{
		...
		{
			glm::mat4 models[1] = glm::translate(Matrix4(1.0f), translations[1]);
